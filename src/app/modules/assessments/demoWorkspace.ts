...
const DEFAULT_TITLE = 'Pulseboard Launch Sprint';
...
const DEFAULT_INSTRUCTIONS = `# Pulseboard Launch Sprint
...
export async function buildDemoWorkspace(input: {
  title: string;
  instructionsMd: string;
}): Promise<StoredAssessmentWorkspace> {
  const loaded = await loadSkeleton('pulseboard-launch-sprint');
  const title = input.title.trim() || DEFAULT_TITLE;
  const instructionsMd = input.instructionsMd.trim() || DEFAULT_INSTRUCTIONS;
  const generatedAt = new Date().toISOString();

  const files = { ...loaded.files };

  // Reconstruct README with interpolated title and instructions
...
`;
  ...
  // Interpolate title into index.html and App.tsx
  files['index.html'] = files['index.html'].replace(DEFAULT_TITLE, title);
...
export function buildDemoAssessmentCopy(input?: {
  ...
  return {
    ...
    instructionsMd: input?.instructionsMd?.trim() || DEFAULT_INSTRUCTIONS,
  };
}
